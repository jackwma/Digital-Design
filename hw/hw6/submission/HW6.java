...
//     r5.repartition(1).saveAsTextFile(output);
  ...
  public static Dataset<Row> Q1 (SparkSession spark, String dataFile) {
    ...
    Dataset<Row> output = spark.sql("SELECT DISTINCT destcityname FROM flights WHERE origincityname = 'Seattle, WA'");
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> r = d.filter(x -> x.get(ORIGIN_CITY_NAME).toString().equals("Seattle, WA"));
    JavaRDD<Row> output = r.map(x -> RowFactory.create(x.get(DEST_CITY_NAME).toString())).distinct();
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> t = d.filter(x -> x.getInt(CANCELLED) != 1);
    JavaPairRDD<Tuple2<String, Integer>, Integer> r = t.mapToPair(x -> new Tuple2<>(new Tuple2<>(x.get(ORIGIN_CITY_NAME).toString(), x.getInt(MONTH)), 1));
    JavaPairRDD<Tuple2<String, Integer>, Integer> output = r.reduceByKey((a,b) -> a+b);
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    JavaPairRDD<String, String> e = d.mapToPair(x -> new Tuple2<>(x.get(ORIGIN_CITY_NAME).toString(), x.get(DEST_CITY_NAME).toString())).distinct();
    JavaPairRDD<String, Integer> f = e.mapToPair(x -> new Tuple2<>(x._1, 1)).reduceByKey((a,b) -> a+b);
    Tuple2<String, Integer> output = f.max(new TupleComparator());
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> e = d.filter(x -> x.getAs(x.fieldIndex("depdelay")) != null);

    JavaPairRDD<String, Tuple2<Double, Integer>> f = e.mapToPair(x -> new Tuple2<>(x.get(ORIGIN_CITY_NAME).toString(), new Tuple2<>((x.getInt(DEP_DELAY)*1.0), 1)));
    JavaPairRDD<String, Tuple2<Double, Integer>> g = f.reduceByKey((a,b) -> new Tuple2<>(a._1() + b._1(), a._2() + b._2()));
    JavaPairRDD<String, Double> output = g.mapToPair((x -> new Tuple2<>(x._1, (x._2._1())/x._2._2())));
